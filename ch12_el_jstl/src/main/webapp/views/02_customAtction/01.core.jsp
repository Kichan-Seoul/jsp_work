...
<%@ page import="java.util.*, vo.Person" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<body>
	...
	* 변수 선언과 동시에 초기화
	...
	<!--  value속성대신 시작태그와 종료태그 사이에 초기값을 지정 가능 -->
	<c:set var="result" scope="request">
		777
	</c:set>
	
	request.result : ${result}<br>
	...
	* 변수 출력 (c:out value="출력하고자하는 값" [default="기본값"] [escapeXml="true|false])
	...
	<c:out value="${outTest}"/><br> <!-- escapeXml 생략시 기본값 true => 문자열로 인식 -->
	...
	* 향상된 for문
	...
	<%
		...
		list.add(new Person("킹키찬", 20, "남자"));
		list.add(new Person("킹대은", 25, "여자"));
		list.add(new Person("킹은하", 23, "여자"));
	%>
