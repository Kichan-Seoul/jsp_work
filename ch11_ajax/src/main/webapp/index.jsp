...
<body>
	...
	<p>
		...
		* 동기식 / 비동기식 <br>
		  > 동기식(a, form, submit)
		  <ul>
		  	<li>요청 처리 후 그에 해당하는 응답페이지가 돌아와야한 그 다음 작업 가능</li>
		  	<li>서버에 요청한 결과까지의 시간이 지연되면 무작정 계속 기다려야 됨(흰 페이지로 보여짐)</li>
		  	<li>전체 페이지를 응답해주기 때문에 기본적으로 페이지가 깜빡거림</li>
		  </ul>
		  > 비동기식(ajax)
		  <ul>
		  	<li>현재 페이지를 기대로 유지하면서 중간중간 추가적으로 필요한 요청을 보내 줄 수 있음</li>
		  ...
		  * 비동기식의 단점 <br>
		  ...
		  - 에러 발생시 디버깅 어려움 <br>
		  - 요청 처리 후에 돌아온 응답데이터를 가지고 현재페이지에 새로운 요소를 만들어서 보여줘야 됨 <br>
	...
	* 부수적인속성
	...
	<form action="idCheck.me">
		ID : <input name="id" id="id"><input type="button" value="ID중복확인" id="btn2"><br>
	...
	<!-- id input에 넣으면 바로바로 쓸 수 있는지 여부 확인 -->
	...
	<script>
		$(()=>{
			...
			$idInput.keyup(function() {
				...
				// 최소 3글자 이상 입력되었을떄만 ajax요청을 중복체크
